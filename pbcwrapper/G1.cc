...
G1 G1::pow2(const Pairing &e, const G1 &base1, const Zr &exp1,
	const G1 &base2, const Zr &exp2)
{
	G1 gout(e, true);
...
G1 G1::pow3(const Pairing &e, const G1 &base1, const Zr &exp1,
	const G1 &base2, const Zr &exp2, const G1 &base3, const Zr &exp3)
{
	G1 gout(e, true);
...
G1::G1(const Pairing &e, const unsigned char *data, unsigned short len,
	bool compressed, unsigned short base): G(e)
{
	if (elementPresent) {
		element_init_G1(g, *(pairing_t*)&e.getPairing());
...
G1::G1(const Pairing &e, string data, unsigned short len): G(e) {
	if (elementPresent) {
		element_init_G1(g, *(pairing_t*)&e.getPairing());
		int hash_len = 20;
